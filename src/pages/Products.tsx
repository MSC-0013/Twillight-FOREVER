      ...
      <div className="relative h-72 sm:h-80 md:h-96 lg:h-[420px] overflow-hidden bg-black">
        ...
        <div className="absolute inset-0 flex items-center justify-center z-20 px-4 sm:px-6 lg:px-8">
          <div className="text-center text-white max-w-4xl">
            ...
            <p className="text-sm sm:text-base md:text-lg lg:text-xl xl:text-2xl mb-4 sm:mb-6 md:mb-8 text-gray-200 animate-fade-in max-w-3xl mx-auto" style={{animationDelay: '0.2s'}}>
              {banners[currentBanner].subtitle}
            </p>
            <Button asChild size="lg" className="bg-white text-black hover:bg-gray-100 text-sm sm:text-base md:text-lg px-4 sm:px-6 md:px-8 py-2 sm:py-3 md:py-4 animate-fade-in shadow-xl hover:shadow-2xl transition-all" style={{animationDelay: '0.4s'}}>
          ...
          {banners.map((_, index) => (
            <button
              ...
              className={`w-2 h-2 sm:w-3 sm:h-3 rounded-full transition-all duration-300 ${
                index === currentBanner ? 'bg-white scale-125 shadow-lg' : 'bg-white/50 hover:bg-white/80'
              }`}
            />
          ))}
        </div>
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8 md:py-12">
            ...
            <div className="lg:col-span-7 grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-3">
              ...
              <div className="sm:col-span-2 lg:col-span-2 flex gap-2">
                <Button
                  ...
                  className={`h-12 flex-1 rounded-xl transition-all ${viewMode === 'grid' ? 'bg-black text-white shadow-lg' : 'border-gray-200 hover:border-black hover:shadow-md'}`}
                >
                ...
                <Button
                  ...
                  className={`h-12 flex-1 rounded-xl transition-all ${viewMode === 'list' ? 'bg-black text-white shadow-lg' : 'border-gray-200 hover:border-black hover:shadow-md'}`}
                >
          ...
          <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between space-y-4 sm:space-y-0">
            ...
            <div className="flex flex-wrap gap-2">
              {selectedCategory !== 'all' && (
                <Badge variant="secondary" className="text-sm px-3 py-1 bg-blue-100 text-blue-800 rounded-full">
                  {categories.find(c => c.id === selectedCategory)?.name}
                </Badge>
              )}
              {searchTerm && (
                <Badge variant="secondary" className="text-sm px-3 py-1 bg-green-100 text-green-800 rounded-full">
                  "{searchTerm}"
                </Badge>
              )}
            </div>
        ...
        {viewMode === 'grid' ? (
          ...
          <div className="space-y-4 sm:space-y-6">
            {filteredProducts.map((product) => (
              <Card key={product.id} className="overflow-hidden hover:shadow-xl transition-all duration-300 border border-gray-200 rounded-xl">
                  ...
                  <div className="flex flex-col md:flex-row gap-4 md:gap-6">
                    ...
                    <div className="flex-1 space-y-3">
                      ...
                      <div className="flex flex-wrap items-center gap-2">
                        <Badge variant="secondary" className="text-xs bg-gray-100 text-gray-800 rounded-full">
                          {product.category}
                        </Badge>
                        ...
                        {product.isNew && (
                          <Badge className="bg-green-500 text-xs rounded-full">New</Badge>
                        )}
                        {product.isFeatured && (
                          <Badge className="bg-blue-500 text-xs rounded-full">Featured</Badge>
                        )}
                      </div>
                      ...
                      <div className="flex flex-col lg:flex-row lg:items-center lg:justify-between space-y-3 lg:space-y-0">
                        <div className="space-y-2">
                          <div className="flex items-center gap-3">
                            <span className="font-bold text-xl md:text-2xl text-black">₹{product.price.toLocaleString()}</span>
                        ...
                        <Button asChild className="bg-black hover:bg-gray-800 text-white rounded-xl shadow-lg hover:shadow-xl transition-all">
                          <Link to={`/products/${product.id}`}>
                            View Details
                          </Link>
                        </Button>
        ...
        {filteredProducts.length === 0 && (
          <div className="text-center py-16 md:py-24">
            ...
            <h3 className="text-2xl md:text-3xl font-bold text-black mb-4">No products found</h3>
