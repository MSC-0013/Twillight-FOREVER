          ...
          <div className="space-y-8">
            <div>
              <div className="flex items-center gap-3 mb-4">
                <Badge variant="outline" className="text-sm font-medium px-3 py-1">{product.brand}</Badge>
            ...
            <div className="bg-white rounded-2xl p-8 shadow-lg border-2 border-gray-100">
              ...
              <div className="mb-6">
                {product.stock > 10 ? (
                  <Badge className="bg-green-100 text-green-700 text-sm px-3 py-2">✓ In Stock - Ready to ship</Badge>
                ) : product.stock > 0 ? (
                  <Badge className="bg-yellow-100 text-yellow-700 text-sm px-3 py-2">
                    ⚠ Only {product.stock} left in stock!
                  </Badge>
                ) : (
                  <Badge className="bg-red-100 text-red-700 text-sm px-3 py-2">✗ Out of Stock</Badge>
                )}
              </div>
              ...
              <div className="space-y-4">
                <div className="flex gap-4">
                  <Button
                    ...
                    className="flex-1 bg-gradient-to-r from-purple-600 to-pink-600 hover:from-purple-700 hover:to-pink-700 text-white font-bold py-4 text-lg rounded-xl"
                    disabled={product.stock === 0}
                  >
                ...
                <Button
                  ...
                  variant="outline"
                  className="w-full py-4 text-lg border-2 border-purple-200 hover:border-purple-400 hover:bg-purple-50 rounded-xl"
                  disabled={product.stock === 0}
                >
            ...
            <div className="bg-white rounded-2xl p-6 shadow-lg border-2 border-gray-100">
              ...
              <div className="space-y-3">
                <div className="flex items-center gap-3"><Truck className="w-5 h-5 text-green-600" /><span>Free shipping on orders over ₹500</span></div>
                <div className="flex items-center gap-3"><Shield className="w-5 h-5 text-blue-600" /><span>2-year warranty included</span></div>
                <div className="flex items-center gap-3"><RotateCcw className="w-5 h-5 text-purple-600" /><span>30-day return policy</span></div>
              </div>
            </div>
          </div>
        </div>
                ...
                <div>
                  ...
                  <div className="space-y-3">
                    {Object.entries(productSpecs).map(([key, value]) => (
                      <div key={key} className="flex justify-between border-b pb-2 border-gray-100">
          ...
          <div>
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-8">
              {relatedProducts.map((relatedProduct) => (
                <Card key={relatedProduct.id} className="group hover:shadow-xl transition-all duration-300 border-2 hover:border-purple-200">
                  <Link to={`/products/${relatedProduct.id}`}>
                    <div className="aspect-square rounded-t-2xl overflow-hidden">
                      <img src={relatedProduct.image} alt={relatedProduct.name} className="w-full h-full object-cover group-hover:scale-110 transition-transform duration-300" />
                    </div>
                    <CardContent className="p-6">
                      <h3 className="font-bold text-lg mb-2 group-hover:text-purple-600">{relatedProduct.name}</h3>
